...
class Student {
    ...
    func mostActive() -> String? {
        let active = academicRecords.max(by: {$0.activityScore < $1.activityScore})
        return  active?.subject.rawValue
    }
...
class Lecturer {
    ...
    var idNumber: Int
    var faculty: Faculty
    var subject: [Subject] // to be changed
    var timeTable: TimeTable // to be changed
...
struct TimeTable {
    ...
    struct DayOfWeek {
        let lectureToBeHeld: LectureToBeHeld
...
//var elene = Student(name: "elene kapanadze", idNumber: 54001056734, faculty: .Architecture, enrolmentYear: 2018, academicRecords: [.init(subject: .Calculus, activityScore: 2.5, midtermScore: 12.7, finalExamScore: 32.8), .init(subject: .Macroeconomics, activityScore: 30, midtermScore: 12.7, finalExamScore: 32.8)])
...
//print(elene.mostActive()!)


var misha = Student(name: "Mikheil Muchaidze", idNumber: 123123, faculty: .BioTechnology, enrolmentYear: 2032, academicRecords: [.init(subject: .Macroeconomics, activityScore: 20.0, midtermScore: 10.0, finalExamScore: 12.0), .init(subject: .Sociology, activityScore: 15.0, midtermScore: 18.0, finalExamScore: 20.0)])

misha.yearsOfStudying(AsOf: 2033)
