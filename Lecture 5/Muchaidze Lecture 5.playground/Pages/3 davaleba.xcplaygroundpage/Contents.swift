...
print("")
let sortedByDoorCount = CarList.sorted { left, right in
    left.carDoorCount < right.carDoorCount
}
sortedByDoorCount.forEach { element in
    print(" karebis raodenoba: \(element.carDoorCount)", element.carBrand, element.carColor, element.fuel.fuelCapacity, element.fuel.fuelCunsumtion)
}
