...
class Car {
    ...
    func fromArrToSting() -> String {
        return "\(self.carBrand), \(self.carColor), \(self.carDoorCount), \(self.fuel.fuelCapacity), \(self.fuel.fuelCunsumtion)"
    }
}
let CarList = [
...
print("")
let dictFromCars = Dictionary(grouping: CarList) { elem in
    elem.carBrand
}
dictFromCars.forEach { key, value in
    print("\(key): \(value.map({$0.fromArrToSting()}))")
}
print("")
